  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 bg-card/50 rounded-lg px-4 py-2 border border-white/10">
        ...
        <div>
          <div className={cn("font-bold", overallLevel.color)}>{overallScore.min}-{overallScore.max}</div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className={cn(
          ...
          <div className={cn("text-4xl font-bold", overallLevel.color)}>
            {overallScore.min}-{overallScore.max}
          </div>
          <div className={cn("text-sm font-medium", overallLevel.color)}>
            {overallLevel.level}
          </div>
          <div className="text-sm text-muted-foreground mt-2">
            Projected {EXAM_CONFIGS[examType].shortName} Score
          </div>
        </div>
